...
function resolveNextStage(currentStage, data) {
    const canEvaluateDocuments = data.ocrCompleted && data.bankingCompleted;
    const canStartCreditSummary = canEvaluateDocuments && data.allDocumentsAccepted;
    switch (currentStage) {
        case "pending":
...
async function advanceProcessingStageInternal(params) {
    ...
    const stageData = {
        ...
        creditSummaryCompleted: Boolean(applicationRecord.credit_summary_completed_at),
