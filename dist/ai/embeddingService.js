...
async function ingestKnowledgeDocument(params) {
    ...
    await (0, db_1.runQuery)(`insert into ai_knowledge_documents (id, filename, category, active, created_at, updated_at)
     values ($1, $2, $3, true, now(), now())`, [documentId, params.filename, params.category]);
    const chunks = chunkText(params.content);
    let count = 0;
    for (const chunk of chunks) {
        const embedding = await generateEmbedding(chunk);
        await (0, db_1.runQuery)(`insert into ai_knowledge_chunks (id, document_id, content, embedding, created_at)
