...
async function uploadDocumentIdempotent(input) {
    const existing = await (0, db_1.runQuery)(`select id, application_id, document_type as category, filename, storage_key as "storageKey", 0::int as size
     from documents
    ...
    const created = await (0, db_1.runQuery)(`insert into documents (id, application_id, owner_user_id, title, document_type, status, filename, storage_key, uploaded_by, created_at, updated_at)
