...
export async function runBankingAnalysis(
  ...
  const allTransactions: Array<BankTransaction & { document_id: string; currency_code: CurrencyCode; account_key: string }> = [];
  const seenStatementBodies = new Set<string>();
  const integrityReport = { duplicate_documents: [] as string[], foreign_currency_documents: [] as string[], opening_balance_rows_removed: 0 };
  ...
  for (const doc of docs) {
    ...
    const statementText = ocrTextFromResult(finalResult);
    const currency = detectStatementCurrency(statementText);
    const fingerprint = statementBodyFingerprint(statementText, transactions);
    if (seenStatementBodies.has(fingerprint)) {
      integrityReport.duplicate_documents.push(doc.documentId);
      transactions = [];
      docError = "Duplicate statement body was excluded from banking totals.";
    } else {
      seenStatementBodies.add(fingerprint);
    }
    const expectedCurrency: CurrencyCode = country === "US" ? "USD" : country === "CA" ? "CAD" : "UNKNOWN";
    if (currency !== "UNKNOWN" && expectedCurrency !== "UNKNOWN" && currency !== expectedCurrency) {
      integrityReport.foreign_currency_documents.push(doc.documentId);
      transactions = [];
      docError = `${currency} statement excluded from ${expectedCurrency} totals; currency conversion is not inferred.`;
    }
    const beforeSanitize = transactions.length;
    transactions = sanitizeTransactions(transactions);
    integrityReport.opening_balance_rows_removed += beforeSanitize - transactions.length;
    ...
    for (const tx of transactions) if (tx.date && Number.isFinite(tx.amount)) allTransactions.push({ ...tx, document_id: doc.documentId, currency_code: currency, account_key: doc.documentId });
  }
...
async function persistAnalysis(applicationId: string, agg: Awaited<ReturnType<typeof aggregateMonthlySummaries>>, llm: { unusualTransactions: any[]; topVendors: any[] }, txCount: number, country: Country, model: string, documentStatuses: BankingDocumentStatus[], integrityReport: object) { await pool.query(`INSERT INTO banking_analyses (application_id, accounts, total_avg_monthly_deposits, average_daily_balance,total_deposits, total_withdrawals, average_monthly_nsfs,months_profitable_numerator, months_profitable_denominator,unusual_transactions, top_vendors, period_start, period_end,months_detected, integrity_report, status, updated_at) VALUES ($1, $2::jsonb, $3, $4, $5, $6, $7, $8, $9, $10::jsonb, $11::jsonb,$12::date, $13::date, $14, $15::jsonb, 'analysis_complete', now()) ON CONFLICT (application_id) DO UPDATE SET accounts = EXCLUDED.accounts,total_avg_monthly_deposits = EXCLUDED.total_avg_monthly_deposits,average_daily_balance = EXCLUDED.average_daily_balance,total_deposits = EXCLUDED.total_deposits,total_withdrawals = EXCLUDED.total_withdrawals,average_monthly_nsfs = EXCLUDED.average_monthly_nsfs,months_profitable_numerator = EXCLUDED.months_profitable_numerator,months_profitable_denominator = EXCLUDED.months_profitable_denominator,unusual_transactions = EXCLUDED.unusual_transactions,top_vendors = EXCLUDED.top_vendors,period_start = EXCLUDED.period_start,period_end = EXCLUDED.period_end,months_detected = EXCLUDED.months_detected,integrity_report = EXCLUDED.integrity_report,status = 'analysis_complete',updated_at = now()`, [applicationId, JSON.stringify([{ note: `${txCount} transactions parsed via ${model} (${country})` }, { documentStatuses }]), agg.avgMonthlyDeposits || null, agg.averageDailyBalance, agg.totalDeposits, agg.totalWithdrawals, agg.averageMonthlyNsfs, agg.monthsProfitable, agg.months, JSON.stringify(llm.unusualTransactions), JSON.stringify(llm.topVendors), agg.periodStart, agg.periodEnd, agg.months, JSON.stringify(integrityReport)]); }
