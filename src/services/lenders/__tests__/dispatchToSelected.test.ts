...
const GoogleSheetSubmissionAdapterMock = vi.fn().mockImplementation(() => ({ submit: submitMock }));

vi.mock("../../../modules/submissions/adapters/GoogleSheetSubmissionAdapter.js", () => ({
  GoogleSheetSubmissionAdapter: GoogleSheetSubmissionAdapterMock,
}));
vi.mock("../../../modules/lenderSubmissions/adapters/EmailAdapter.js", () => ({ sendLenderEmail: vi.fn() }));
vi.mock("../buildApplicationPackage.js", () => ({ buildApplicationPackage: vi.fn().mockResolvedValue({ zipBuffer: Buffer.from("zip") }) }));
vi.mock("../loadPackageInputs.js", () => ({ loadPackageInputs: vi.fn().mockResolvedValue({ signedApplicationPdf: Buffer.from("signed"), creditSummaryPdf: Buffer.from("credit"), documents: [{ category: "bank", files: [{ filename: "stmt.pdf", content: Buffer.from("doc") }] }], fields: [{ label: "Business Name", value: "Acme LLC" }] }) }));
...
describe("dispatchToSelected", () => {
  ...
  it("instantiates GoogleSheetSubmissionAdapter for google_sheet method", async () => {
    const { dispatchToSelected } = await import("../dispatchToSelected.js");
    const pool = { query: vi.fn().mockResolvedValue({ rowCount: 1 }) } as any;
    const sent = await dispatchToSelected({ pool, applicationId: "app-2" }, [{ lender_id: "lender-2", name: "Sheet Lender", submission_method: "google_sheet", submission_email: null, api_endpoint: null, api_key_encrypted: null, google_sheet_id: "sheet-123" }]);
