...
async function hasTable(table: string): Promise<boolean> {
  const res = await runQuery<{ count: number }>(
...
export async function runStartupConsistencyCheck(): Promise<void> {
  ...
  try {
    ...
    if (!hasDocuments || !hasApplications || !hasDocumentApplicationId) {
      ...
      const orphanDocumentsCount = await runQuery<{ count: number }>(
        `select count(*)::int as count
         from documents d
         where not exists (
      ...
      const orphanDocumentsSample = await runQuery<SampleRow>(
    ...
    if (!hasApplications || !hasUsers || !hasOwnerUserId) {
      ...
      const orphanApplicationsCount = await runQuery<{ count: number }>(
        `select count(*)::int as count
         from applications a
         where not exists (
      ...
      const orphanApplicationsSample = await runQuery<SampleRow>(
    ...
    if (!hasApplications || !hasPipelineState) {
      ...
      const invalidPipelineCount = await runQuery<{ count: number }>(
        `select count(*)::int as count
         from applications
         where not (pipeline_state = any($1::text[]))`,
        [PIPELINE_STATES]
      );
      const invalidPipelineSample = await runQuery<SampleRow>(
