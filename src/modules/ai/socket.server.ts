...
async function attachTranscriptToCrm(sessionId: string): Promise<void> {
  const sessionResult = await runQuery<{ lead_id: string | null }>(
  ...
  const messages = await runQuery<{ role: string; message: string | null; content: string | null; created_at: string }>(
