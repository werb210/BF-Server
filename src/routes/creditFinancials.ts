...
const svc = () => import("../services/credit/financials.js");
...
router.get("/:applicationId", ...staff, safeHandler(async (req: any, res: any) => {
  res.json({ table: await (await svc()).loadFinancialTable(String(req.params.applicationId)) });
}));

router.post("/:applicationId/extract", ...staff, safeHandler(async (req: any, res: any) => {
  const applicationId = String(req.params.applicationId);
  try {
    const result = await (await svc()).extractApplicationFinancials(applicationId);
    console.info("[credit-financials] extracted", { applicationId, ...result });
    res.json({ ...result, table: await (await svc()).loadFinancialTable(applicationId) });
  } catch (error) {
...
router.put("/:applicationId/cell", ...staff, safeHandler(async (req: any, res: any) => {
  ...
  try {
    await (await svc()).setFinancialCell(applicationId, { period: body.period, kind: body.kind, periodEnd: body.periodEnd, item: body.item, value }, req.user?.id ? String(req.user.id) : null);
  } catch (error) {
    return res.status(400).json({ error: (error as Error).message, message: "Unknown period or line item." });
  }
  res.json({ table: await (await svc()).loadFinancialTable(applicationId) });
}));
