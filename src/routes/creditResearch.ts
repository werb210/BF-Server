...
import { loadResearch, refreshApplicationResearch, setFactStatus } from "../services/credit/research.js";
...
router.get("/:applicationId", ...staff, safeHandler(async (req: any, res: any) => {
  res.json(await loadResearch(String(req.params.applicationId)));
}));

router.post("/:applicationId/refresh", ...staff, safeHandler(async (req: any, res: any) => {
  const applicationId = String(req.params.applicationId);
  try {
    const result = await refreshApplicationResearch(applicationId, req.query?.force === "1");
    console.info("[credit-research] refreshed", { applicationId, ...result });
    res.json({ ...result, ...(await loadResearch(applicationId)) });
  } catch (error) {
...
router.put("/:applicationId/facts/:factId", ...staff, safeHandler(async (req: any, res: any) => {
  const applicationId = String(req.params.applicationId);
  try {
    const updated = await setFactStatus(applicationId, String(req.params.factId), String(req.body?.status ?? ""), req.user?.id ? String(req.user.id) : null);
    if (!updated) return res.status(404).json({ error: "not_found" });
  } catch {
    return res.status(400).json({ error: "bad_status", message: "Status must be confirmed, rejected or unverified." });
  }
  res.json(await loadResearch(applicationId));
}));
