...
function runMigrationsUpTo133FromFreshDb() {
  const db = newDb();
...
describe("133 migration on fresh DB", () => {
  it("creates companies schema/indexes/fk/checks without over-registering migrations", () => {
    const db = runMigrationsUpTo133FromFreshDb();
