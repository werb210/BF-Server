...
const nextPhone = (): string => `+1415777${String(phoneCounter++).padStart(4, "0")}`;
...
async function seedLenderProduct(): Promise<{ lenderId: string; lenderProductId: string }> {
  ...
  await pool.query(
    ...
    [lenderId, "Visibility Lender", "US", "email", "submissions@visibility.com", null]
  );
  await pool.query(
...
async function seedApplication(params: { lenderId: string; lenderProductId: string; ownerUserId: string }) {
...
describe("client submission visibility", () => {
  it("marks the application as submitted to the lender", async () => {
    const staffUser = await createUserAccount({
      email: "visibility@apps.com",
    ...
    const { rows } = await pool.query(
      "select status, pipeline_state from applications where id = $1",
      [applicationId]
    );
    expect(rows[0].status).toBe("SUBMITTED_TO_LENDER");
    expect(rows[0].pipeline_state).toBe("OFF_TO_LENDER");
  });
});
